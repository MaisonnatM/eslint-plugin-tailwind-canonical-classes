...
function getDesignSystemSync(cssPath, context) {
  ...
  if (designSystemPromise && !designSystemCache) {
    // Promise is loading but not resolved yet - can't block synchronously
    return null;
  }
...
export default {
  ...
  create(context) {
    ...
    // Start loading the design system immediately
    getDesignSystemSync(cssPath, context);
    const preloadPromise = getDesignSystemAsync(cssPath, context);

    return {
      // Use Program visitor to ensure design system is loaded before processing any JSX
      // This is critical for ESLint's --fix mode to work properly
      async Program() {
        // Eagerly load the design system when the file starts processing
        // This ensures it's ready before any JSXAttribute visitors run
        try {
          await preloadPromise;
        } catch (error) {
          // Error will be handled when trying to use the design system
          // Don't throw here to avoid stopping ESLint processing
        }
      },
      ...
      async JSXAttribute(node) {
        ...
        // Ensure design system is loaded before processing
        // This await is critical for --fix to work properly
        let designSystem;
        try {
          await preloadPromise;
          designSystem = getDesignSystemSync(cssPath, context);
          
          // If still not loaded, wait for async
          if (!designSystem) {
            designSystem = await getDesignSystemAsync(cssPath, context);
          }
        } catch (error) {
          // Design system failed to load, skip processing
          return;
        }
